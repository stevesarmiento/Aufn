...
///
/// The drag is a UIKit pan (`HorizontalPanGesture`) so UIKit arbitrates it
...
struct SwipeRow<Content: View>: View {
    ...
    private var canDelete: Bool { !inSelectionMode }
    private var isOpen: Bool { openRowID == id }
    private var baseOffset: CGFloat { isOpen ? -SwipeRules.revealWidth : 0 }
    private var offset: CGFloat {
        SwipeRules.offset(base: baseOffset, translation: translation, canDelete: canDelete, canSelect: true)
    }
    ...
    private var selectArmed: Bool { offset >= SwipeRules.revealWidth }
    private var deleteArmed: Bool { offset <= -SwipeRules.revealWidth }
    ...
    var body: some View {
        content()
            ...
            .overlay {
                ...
                if inSelectionMode {
                    Color.clear
                        ...
                        .onTapGesture {
                            withAnimation(.snappy) { toggleSelection() }
                        }
                }
            }
            ...
            .accessibilityAction(named: Text(deleteAccessibilityLabel)) {
                if canDelete { confirmingDelete = true }
            }
    ...
    private var deletePanel: some View {
        let progress = SwipeRules.fillProgress(travel: deleteTravel)
    ...
    private var selectPanel: some View {
        let progress = SwipeRules.fillProgress(travel: selectTravel)
    ...
    private var pan: HorizontalPanGesture {
        HorizontalPanGesture(
            ...
            onEnded: { x, _ in
                let outcome = SwipeRules.outcome(
                    base: baseOffset,
                    translation: x,
                    canDelete: canDelete,
                    canSelect: true
